...
const AddTvShowToWatchLater = ({ result, resetActive }) => {
    ...
    const handleAddToWatchlist = (event, newRating) => {
        ...
        fetch(`/add-tvshow-to-watchlater/${currentUser}`, {
            method: "PATCH",
            headers: {
            ...
            .then((parse) => {
            if (parse.status === 200) {
                window.alert("Show added to the Watch later");
            }
            if (parse.status === 404) {
                window.alert("Please sign in to add shows to your Watch list!")
                setActive(false)
            }
            if (parse.status === 400) {
                window.alert("Please sign in to add shows to your Watch list!")
...
export default AddTvShowToWatchLater;